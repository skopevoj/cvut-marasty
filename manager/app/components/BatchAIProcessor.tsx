...
export function BatchAIProcessor({
    ...
    const [stats, setStats] = useState({ total: 0, processed: 0, confirmed: 0, rejected: 0 });

    // Editable state for current question
    const [editingQuestion, setEditingQuestion] = useState<ProcessedQuestion['parsed'] | null>(null);
    ...
    async function startBatchProcessing() {
        setProcessing(true);
        setStats({ total: selectedImages.length, processed: 0, confirmed: 0, rejected: 0 });

        // Initialize questions
        const initialQuestions: ProcessedQuestion[] = selectedImages.map((imageName) => ({
            imageName,
            imageUrl: '',
    ...
    async function processImage(imageName: string, index: number) {
        ...
        try {
            ...
            const imageRes = await fetch(`/api/fs?action=readImage&path=${encodeURIComponent(imagePath)}`);
            ...
            const mergedTopics = Array.from(new Set([...forcedTopics, ...(parsed.topics || [])]));
    ...
    async function handleConfirm() {
        ...
        try {
            ...
            // Save question with edited data
            await fetch('/api/fs', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            const imagePath = `${folderPath}/${subject.code}/unprocessed/${current.imageName}`;
            const imageRes = await fetch(`/api/fs?action=readImage&path=${encodeURIComponent(imagePath)}`);
            const blob = await imageRes.blob();
        ...
        <div className="h-full flex flex-col">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                {stats.processed}/{stats.total} processed • {stats.confirmed} confirmed • {stats.rejected}{' '}
                                rejected
                            </p>
                        </div>
                    </div>
            ...
            <div className="flex-1 overflow-auto p-6">
                <div className="max-w-5xl mx-auto">
                    {processing && questions.length > 0 && stats.processed < stats.total && (
                        <div className="text-center py-12">
                            <Loader2 className="w-12 h-12 animate-spin text-primary mx-auto mb-4" />
                            <p className="text-lg font-semibold mb-2">Processing with AI...</p>
                    ...
                    {!processing && questions.length === 0 && (
                        <div className="text-center py-12">
                            ...
                            <p className="text-muted-foreground">
                                Click "Start Processing" to parse all unprocessed images with AI.
                            </p>
                            <p className="text-sm text-muted-foreground mt-2">
                                {subject.unprocessedImages?.length || 0} images will be processed
                            </p>
                        </div>
                    )}

                    {questions.length > 0 && stats.processed >= stats.total && current && (
                        <div className="space-y-6">
                            ...
                            {current.status === 'error' ? (
                                <div className="p-6 bg-destructive/10 border border-destructive rounded-xl text-center">
                                    ...
                                    <p className="text-destructive font-semibold">Failed to process this image</p>
                                    <p className="text-sm text-muted-foreground mt-2">{current.error}</p>
                                    <button
                                ...
                                <>
                                    ...
                                    {current.imageUrl && (
                                        <div className="bg-card border border-border rounded-xl overflow-hidden">
                                            <img src={current.imageUrl} alt={current.imageName} className="w-full h-auto" />
                                        </div>
                                    )}
                                    ...
                                    {editingQuestion && (
                                        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) =>
                                                        setEditingQuestion({
                                                            ...editingQuestion,
                                                            questionType: e.target.value as 'multichoice' | 'open',
                                                        })
                                                    }
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-2">
                                                    {subject.topics?.map((topic) => (
                                                        <button
                                                            ...
                                                            type="button"
                                                            onClick={() => {
                                                                const isSelected = editingQuestion.topics.includes(topic.id);
                                                                setEditingQuestion({
                                                                    ...editingQuestion,
                                                                    topics: isSelected
                                                                        ? editingQuestion.topics.filter((t) => t !== topic.id)
                                                                        : [...editingQuestion.topics, topic.id],
                                                                });
                                                            }}
                                                            className={`px-3 py-1.5 text-sm rounded-lg border transition-all ${editingQuestion.topics.includes(topic.id)
                                            ...
                                            {editingQuestion.answers && editingQuestion.answers.length > 0 && (
                                                <div>
                                                    <label className="text-sm font-medium text-muted-foreground">Answers</label>
                                                                ...
                                                                <div className="flex items-start gap-3">
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => {
                                                                            ...
                                                                            newAnswers[i] = { ...newAnswers[i], isCorrect: e.target.checked };
                                                                    ...
                                                                    <button
                                                                        onClick={() => {
                                                                            const newAnswers = editingQuestion.answers!.filter((_, idx) => idx !== i);
                                                                            setEditingQuestion({ ...editingQuestion, answers: newAnswers });
                                                                        }}
