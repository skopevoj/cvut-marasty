...
export async function POST(req: NextRequest) {
  try {
    const { action, folderPath, data, filePath, imageQuality } =
      await req.json();
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        const subjectJsonPath = path.join(
          folderPath,
          entry.name,
          "subject.json",
        );
        try {
          ...
          const unprocessedDir = path.join(
            folderPath,
            entry.name,
            "unprocessed",
          );
...
async function loadQuestions(questionsDir: string) {
  ...
  try {
    const questionFolders = await fs.readdir(questionsDir, {
      withFileTypes: true,
    });
...
async function loadUnprocessedImages(
  dir: string,
  relativePath: string,
): Promise<string[]> {
  const images: string[] = [];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const subImages = await loadUnprocessedImages(
          fullPath,
          newRelativePath,
        );
...
async function saveSubject(folderPath: string, data: any) {
  ...
  if (originalTopics && subjectData.topics) {
    ...
    originalTopics.forEach((oldTopic: any) => {
      const newTopic = subjectData.topics.find(
        (t: any) => t.name === oldTopic.name,
      );
    ...
    const subjectPromises = entries
      ...
      .map(async (entry) => {
        ...
        try {
          ...
          const questionsDir = path.join(folderPath, entry.name, "questions");
          const questions = await loadQuestionsWithImages(
            questionsDir,
            imageQuality,
          );
...
async function loadQuestionsWithImages(
  questionsDir: string,
    ...
    const questionPromises = questionFolders
      ...
      .map(async (folder) => {
        ...
        try {
          ...
          // Process both possible images in parallel for this question
          const [image, quizPhoto] = await Promise.all([
            (async () => {
              const imageFile = files.find((f) =>
                /^image\.(png|jpg|jpeg|gif|webp)$/i.test(f),
              );
              if (imageFile) {
            ...
            (async () => {
              const quizImageFile = files.find((f) =>
                /^(quizImage|quizPhoto)\.(png|jpg|jpeg|gif|webp)$/i.test(f),
              );
              if (quizImageFile) {
