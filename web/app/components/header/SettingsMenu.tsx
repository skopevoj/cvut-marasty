...
export function SettingsMenu() {
  ...
  return (
    <div className="flex flex-col gap-5 max-h-[80vh] overflow-y-auto pr-2">
      <div>
        ...
        <div className="space-y-3">
          {settings.dataSources.map((source) => (
            <div
              key={source.id}
              className="flex items-center justify-between p-4 rounded-3xl bg-[var(--bg-elevated)]/40 border border-[var(--border-default)] backdrop-blur-sm hover:border-[var(--border-hover)] transition-colors"
            >
              <div className="flex items-center gap-3 min-w-0">
                <div
                  className={`p-2.5 rounded-2xl transition-all ${source.enabled ? "bg-[var(--subject-primary)]/15 text-[var(--subject-primary)]" : "bg-[var(--bg-surface)]/50 text-[var(--fg-muted)]"}`}
                >
                  {source.type === "url" ? (
                    <Globe size={18} />
                  ) : (
                ...
                <div className="min-w-0">
                  <div className="text-sm font-medium truncate leading-none mb-1 text-[var(--fg-primary)]">
                    {source.name}
                  </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full border-2 transition-all ${source.enabled ? "bg-[var(--subject-primary)] border-[var(--subject-primary)]" : "border-[var(--border-default)]"}`}
                  />
                </button>
                <button
          ...
          {settings.dataSources.length === 0 && (
            <p className="text-center py-6 text-sm text-[var(--fg-muted)] italic">
              Žádné zdroje dat
            </p>
          )}
        </div>
      ...
      <SettingsToggle
        label="Kontrola nových otázek při startu"
        description="Pokud vypnuto, použijí se otázky z cache."
        icon={<BarChart2 size={24} />}
        isActive={settings.checkUpdatesOnStartup}
