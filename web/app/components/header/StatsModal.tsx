...
import { useDataStore, useStatsStore } from "../../lib/stores";
...
export function StatsModal() {
  ...
  const graphData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayAttempts = stats.attempts.filter(
        (a) =>
    ...
    return days;
  }, [stats.attempts, currentSubject, questions]);

  // Categories Data
  const categoriesData = useMemo(() => {
    if (!currentSubject) return [];

    const subjectAttempts = stats.attempts.filter(
      (a) => a.subjectCode === currentSubject.code,
    );
    const topics = helpers.getAvailableTopics(
      questions,
      currentSubject,
      topicMap,
    );
    ...
    return topics
      .map((topic) => {
        const topicAttempts = subjectAttempts.filter(
          (a) => a.topics?.includes(topic.id) || a.topic === topic.id,
        );

        if (topicAttempts.length === 0)
          return { ...topic, percentage: 0, total: 0 };
  ...
  }, [stats.attempts, currentSubject, questions, topicMap]);
  ...
  return (
    <div className="flex flex-col gap-8 py-2">
      {/* Chart Area */}
      <div>
        <h3 className="mb-6 text-[10px] font-bold text-text-secondary uppercase tracking-[0.2em]">
          Aktivita za poslední týden
        </h3>
            ...
            <div key={i} className="flex flex-1 flex-col items-center gap-3">
              <div className="relative w-full flex flex-col items-center justify-end h-32 group">
                ...
                <div className="w-full max-w-[12px] md:max-w-[16px] bg-white/5 rounded-full relative overflow-hidden h-full">
                  <div
                    className="absolute bottom-0 left-0 right-0 bg-white/10 rounded-full"
              ...
              <span className="text-[9px] font-bold text-text-secondary uppercase tracking-tight">
                {day.date}
              </span>
            </div>
          ))}
            ...
            <div key={i} className="space-y-2">
              <div className="flex justify-between text-[11px] px-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] text-text-secondary opacity-40 tabular-nums">
                    ({cat.total})
                  </span>
                </div>
              </div>
          ...
          {categoriesData.length === 0 && (
            <div className="text-center py-12 bg-white/5 rounded-2xl border border-dashed border-white/10">
              <p className="text-text-secondary text-xs font-medium">
                Zatím žádná data pro tento předmět
              </p>
            </div>
          )}
        </div>
