import type { Question, Answer, QuestionAttempt } from "../types";
...
export const statsHelper = {
  ...
  isAttemptCorrect(
    ...
    if (attempt.type === "open") {
      ...
      const userAnswer =
        typeof attempt.userAnswers === "string"
          ? attempt.userAnswers.trim()
          : "";
      return correctAnswers.includes(userAnswer);
    } else {
      const userAnswers = attempt.userAnswers as Record<
        number,
        boolean | number
      >;
      return question.answers.every((ans: Answer, i: number) => {
        ...
        return (
          (userChoice === 1 && isActuallyCorrect) ||
          (userChoice === 3 && !isActuallyCorrect)
        );
      });
    }
  },
  ...
  getStatsByQuestionId(
    questionId: string,
    questions: Question[],
    customAttempts?: QuestionAttempt[],
  ) {
    const attempts = (customAttempts || this.getAttempts()).filter(
