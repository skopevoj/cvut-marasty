...
interface SettingsContextType {
    ...
    addDataSource: (source: Omit<DataSource, 'id' | 'enabled'> & { data?: any }) => void;
...
export function SettingsProvider({ children }: { children: ReactNode }) {
                    ...
                    if (parsed.dataSources) {
                        for (const source of parsed.dataSources) {
                            if (source.type === 'local' && source.data) {
                                console.log(`Migrating data for source ${source.name} to IndexedDB...`);
                                try {
    ...
    const addDataSource = async (source: Omit<DataSource, 'id' | 'enabled'> & { data?: any }) => {
